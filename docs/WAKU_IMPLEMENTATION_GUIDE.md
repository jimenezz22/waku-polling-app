...
function DecenVoteApp() {
  ...
  // Light Push hooks
  const { push: pushPoll } = useLightPush({ node, encoder: pollEncoder });
  const { push: pushVote } = useLightPush({ node, encoder: voteEncoder });

  // Filter hooks for real-time messages
  const { messages: livePolls } = useFilterMessages({ node, decoder: pollDecoder });
  const { messages: liveVotes } = useFilterMessages({ node, decoder: voteDecoder });

  // Store hooks for historical messages
  const { messages: historicalPolls } = useStoreMessages({ node, decoder: pollDecoder });
  const { messages: historicalVotes } = useStoreMessages({ node, decoder: voteDecoder });

  // Initialize identity
  useEffect(() => {
    const userIdentity = getOrCreateIdentity();
    setIdentity(userIdentity);
  }, []);

  // Process historical and live polls
  useEffect(() => {
    const allPollMessages = [...historicalPolls, ...livePolls];
    const decodedPolls = allPollMessages
  ...
  // Process historical and live votes
  useEffect(() => {
    const allVoteMessages = [...historicalVotes, ...liveVotes];
    const decodedVotes = allVoteMessages
      .filter(msg => msg.payload)
      .map(msg => {
        try {
          return VoteMessage.decode(msg.payload);
  ...
  const createPoll = async (question, options) => {
    ...
    const { recipients, errors } = await pushPoll({
      payload,
    ...
    if (errors.length === 0) {
      ...
      console.error("Failed to create poll:", errors);
    }
  };

  // Cast a vote
  const castVote = async (pollId, optionIndex) => {
    if (!pushVote || !identity) return;

    const voteData = {
      pollId,
      optionIndex,
      voterPublicKey: identity.publicKeyHex,
      signature: "", // Would implement proper signing
      timestamp: Date.now()
    };
    ...
    const { recipients, errors } = await pushVote({
      payload,
    ...
    if (errors.length === 0) {
      ...
      console.error("Failed to cast vote:", errors);
    }
  };

  if (isLoading) return <div>Loading Waku node...</div>;
  if (error) return <div>Error: {error}</div>;

  return (
    <div className="decenvote-app">
      <h1>DecenVote</h1>
      <div>Identity: {identity?.publicKeyHex?.slice(0, 16)}...</div>

      {/* Poll creation form */}
      <PollCreationForm onCreatePoll={createPoll} />

      {/* Polls list */}
      <PollsList polls={polls} votes={votes} onVote={castVote} />
    </div>
  );
}
```
